...
public class LongestRepeatingSubsequence {
    ...
    // Approach: We have to apply the LCS approach itself by applying LCS on screen with itself
    // We compute LCS of the string with itself, but prevent matching a character with itself using i != j, ensuring only repeated characters at different positions are counted.
    // TC => O(n^2)
    // SC => O(n^2)
    public int LongestRepeatingSubsequence(String s) {
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = 1; j <= n; j++) {
                if(s.charAt(i - 1) == s.charAt(j - 1) && i != j) // ensures that the repeated characters at different positions are counted
                    dp[i][j] = 1 + dp[i - 1][j - 1];
                else
